...
def get_portfolio():
    try:
        holdings_raw = getattr(portfolio_manager, "assets", {})
...
def get_prices():
    ...
    try:
        ...
        if not symbols_param:
            return jsonify({"error": "query parameter 'symbols' required"}), 400
        ...
        # ⬇️ Moved OUTSIDE the loop (correct)
        response = {
            "timestamp": datetime.now().isoformat(),
            "prices": prices
        }
